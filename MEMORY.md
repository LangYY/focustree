...
## 2026-08-12 — 树标签：胶囊衬底与整树 3D 透视双双否掉，改为「移到节点上方 + 去描边」

- **胶囊衬底（上一条）在真实环境里不成立，已整体删除**。两版都试过：沿用 `--ft-surface-hover` 时，该 token 与 `--ft-canvas` 在两个主题下几乎同色（晨纸 `#F0EDE3` vs `#F3F0E7`，林夜 `#1F281E` vs `#0B0F0C`），再乘 `.42`/`.34` opacity 后视觉上等于不存在——上一条记录的「浏览器校对通过」是在看一个几乎不可见的东西。改用 `--ft-canvas` 填充并提到 `.92`/`.88` 后确实可见了，但在枝干上留下圆角矩形的「橡皮擦」缺口，正是 DESIGN.md 要避免的破坏有机感。结论：标签背后不加任何几何衬底。
- **根因不在衬底，在位置**。标签固定在节点右侧，而右侧正是子枝干的出发方向，重叠是设计上必然的。此前三轮（描边光晕 → 配色收敛 → 胶囊衬底）都在给这个必然的重叠打补丁。改为放在节点正上方（兄弟节点之间稳定空着的区域）后重叠从根上消失，密度全开 35 个标签实测重叠对数为 0。
- **避让方向必须随之反转**：旧算法从上往下排、碰撞时往下推；标签在上方时「往下」就是压到自己的节点上。改成从画布最下方往上依次放置、碰撞时把上面那个继续往上顶。首次实现时测试断言写反（以为被推的是下面那个），是跑出来才发现方向错的。
- **文字描边（`paint-order: stroke fill` + 2px 画布色）是「字有边框」的真凶**，已删除。2px 描边在字形外侧留 1px 画布色轮廓，而中文 16px 下笔画间距只有 1–2px，轮廓会糊进笔画空隙，视觉上等于给每个字加边框。改用 `drop-shadow` 柔光，颜色走新增的 `--ft-label-halo`：林夜暗影、晨纸画布色柔光（浅底上文字是深色，暗影只会让字更脏，两个主题不能共用一个值）。
- **整树 3D 透视（CSS `rotateY`）实测不可用**，不要再尝试这条路。视觉成立，但 D3 的 `getScreenCTM()` 不认识 CSS 3D 变换，指针映射误差随深度增长：仅 14° 倾斜下第 1 层约 17px、第 2 层 40px、第 3 层 72px、第 4 层 117px，而拖拽阈值是 4px、拖出创建阈值是 28px。DOM 命中检测（点击/悬浮/右键）能正常穿过 3D 变换，但所有走坐标计算的交互都会坏。若要纵深感，正确做法是把透视缩放写进布局（作用于节点坐标和半径，保持纯 2D SVG），而不是加 CSS 变换。
- **字体统一**：删除 `--ft-font-serif` 及其全部 15 处用法，全应用只剩 `--ft-font-sans`，层级改由字号字重承担。此前误判过「衬线回退成 SimSun」，实测（宽度指纹）该机装有 Noto Serif SC 且实际生效——问题不是回退，是衬线中文横画极细，在深色底、小字号、DPR 1.5 非整数缩放下会发灰断裂。
- **顺带修掉的死代码**：标签的 `click`/`dblclick` 一直被 `src/index.css` 的 `.node text { pointer-events: none }` 屏蔽，从未触发；实际靠向右延伸 `DROP_LABEL_WIDTH` 的 `.node-hit-area` 兜底。标签移到上方后不在那块区域里，因此给 `.node-label` 单独放开 pointer-events。`branchTangent` 失去全部调用方，已删除（`cubicTangent` 仍有其他调用方，保留）。

---

## 2026-08-12 — 载入示例数据会无声删光真实数据（真实发生过一次）

- `useTree.js` 的 `loadExampleData` 先把该用户全部节点从 Supabase **硬删除**再插入示例，且**不进撤销栈**。新手引导的「先看个示例」分支与账户菜单的「载入示例数据」都直接调用它，两处都没有确认。本次开发中用户点「重新看一遍引导」后选了示例，43 个真实节点（含 39 条 annotations、59 条对话）当场消失，撤销键救不回来。
- 数据可恢复：`useBackup` 的每小时自动备份把删除前的完整快照留在 localStorage（`ft_backup_<uid>_<ts>`，含 nodes/annotations/conversations 等 10 张表），5 个快照全部早于删除。恢复入口是账户菜单 →「数据备份」的 `fullRestore`。本次用户选择不恢复。
- 修复：树非空时先 `window.confirm` 明示会删除多少节点且无法撤销，确认后先 `preDestructiveBackup` 快照再删；空树（真正的新用户）不打扰。两个入口汇流到 `App.jsx` 的同一个 `loadExample`，一道防护覆盖两处。
- `test/exampleDataGuard.test.js` 钉住确认→快照→删除的**先后顺序**、拒绝确认时必须直接返回，以及「`loadExampleData` 确实会删」这个前提——哪天它改成不删了，这道防护应该被重新评估而不是默默留着。
- 教训（工具使用层面）：不要用脚本对含破坏性选项的菜单做盲点击。本次事故并非我的脚本触发，但当时确实在未截图确认菜单结构的情况下程序化点击了账户菜单，而「载入示例数据」「清空全部」就紧邻目标项。后续改为按文本精确匹配并先校验再点。

---

## 2026-08-12 — 新手引导重做：先讲产品，再要输入

- 用户反馈的核心不是流程卡顿，而是「没有具体引导，没有给用户第一印象这个产品是用来做什么的」。旧第一屏直接是「把现在脑子里的事，放到一棵树上」加输入框。
- 前面补两拍：①这是什么（痛点+收益一句话）；②怎么读这棵树（三个视觉通道逐个讲，示意图跟着变：加辉光 / 枝干 2.5→7 / 画年轮）。DESIGN.md §6.2 早写着「三通道不解释没人能读懂」，但旧引导一字未提，用户走完只能得到一棵看不懂的树。示意图是自带极简 SVG，不读真实数据（新用户此刻没有树，示例用户的树也不该被盖住）。
- 修掉两个会**永久卡死**的等待态：`WAITING` 原先只认「带提案的回复」，AI 回闲聊、反问或请求失败就永远停住，只能跳过；现在无提案转 `RETRY`（提示换说法并给例句），45s 超时也转 `RETRY`，迟到的提案仍接得回主线。`RECOMMENDATION_WAITING` 同样加超时。
- 补后退与进度：`canGoBack` 明确规定哪些步骤可退——已写库或已发出请求的不给退。
- **仍未处理**：「先看个示例」分支依旧跳过核心循环（说→提案→确认→树生长），只是现在会先看到前两拍。是否让它用示例数据也演示一遍，是待定的产品决定。
