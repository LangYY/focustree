...
1. 组装 system prompt：当前树、阶段目标、近期摘要、用户模式、命中率、时间。
2. 选择模型。
3. 调用 LLM。
4. 剥离 markdown 包裹并解析 JSON。
5. 标准化字段名。
6. 校验 action 类型、真实 node id、枚举值。
7. 失败时把错误反馈给模型重试，最多 3 次。
8. 仍失败则返回安全降级结果，不改树。
...
4. 如果草案节点尚未创建，执行允许的 `draft_actions`。
