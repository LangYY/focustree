...
FocusTree 是给多线程个体工作者使用的外脑：左侧是一棵可操作的项目树，右侧是一个能一起思考的 Focus Agent。
...
这些由本地确定性算法直接执行。Focus Agent 真正负责的是：
...
- 状态、期限、人工优先级、结构、关键路径传播、完成痕迹和分析过期判断全部本地完成，0 token。
- 只有目标契合、必要性、延误损失和父子关系类型需要模型判断。
- “补充 AI 分析”只发送缺失或过期的未完成节点；“全树重新分析”发送全部未完成节点。
- 全树分析使用独立的精简接口并分批并行调用，不携带通用 Agent 的对话历史、few-shot 和项目操作规则。窄任务优先使用快速模型，只有结构校验失败的批次才升级到深度模型。
- 模型结果只进入待确认卡片；用户确认前不改变节点分数。
