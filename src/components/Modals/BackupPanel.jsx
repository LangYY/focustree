...
export default function BackupPanel({
  ...
  return (
    ...
    >
      <div
        className="ft-modal-panel border rounded-2xl w-[640px] max-h-[85vh] flex flex-col shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-5 py-3 border-b ft-modal-border-subtle flex items-center justify-between">
          <div>
            <div className="text-base font-semibold ft-modal-text-primary">数据备份与恢复</div>
            <div className="text-[11px] ft-modal-text-tertiary mt-0.5">
              你的数据可以随时导出、恢复，不怕意外丢失
            </div>
          </div>
          <button onClick={onClose} className="ft-modal-text-tertiary ft-modal-hover-text-secondary text-sm">关闭 ✕</button>
        </div>

        {/* Status row */}
        <div className="px-5 py-3 border-b ft-modal-border-subtle ft-modal-surface-base-60 space-y-1.5 text-[12px]">
          <div className="flex items-center gap-2">
            <span className="ft-modal-text-accent">●</span>
            <span className="ft-modal-text-secondary">自动备份已启用</span>
            <span className="ft-modal-text-tertiary">·</span>
            <span className="ft-modal-text-secondary">每小时一次，存在浏览器本地</span>
          </div>
          <div className="flex items-center gap-3 ft-modal-text-secondary">
            <span>上次自动: {lastAuto ? formatRelative(new Date(lastAuto).toISOString()) : '尚未'}</span>
            <span className="ft-modal-text-tertiary">·</span>
            <span>上次手动导出: {lastManual ? formatRelative(new Date(lastManual).toISOString()) : <span className="ft-modal-text-warn">从未</span>}</span>
          </div>
        </div>

        {/* Manual export / import */}
        <div className="px-5 py-4 border-b ft-modal-border-subtle space-y-2.5">
          <button
            onClick={onExport}
            disabled={working}
          ...
          />
          {progress && (
            <div className="text-xs ft-modal-text-secondary px-1">{progress}</div>
          )}
        </div>
  ...
  return (
    <div>
      ...
      {items.length === 0 && emptyText && (
        <div className="text-xs ft-modal-text-faint px-1">{emptyText}</div>
      )}
      <div className="space-y-1.5">
        {items.map(b => {
          ...
          return (
            <div
              key={b.key}
              className="flex items-center justify-between px-3 py-2 ft-modal-surface-hover-40 ft-modal-hover-surface-60 rounded-lg border ft-modal-border-subtle transition-colors"
            >
              <div className="min-w-0">
                <div className="text-xs ft-modal-text-secondary">
                  {when}
                  {meta?.reason && meta.reason !== 'periodic' && (
                    <span className="text-[10px] ft-modal-text-tertiary ml-2">· {meta.reason}</span>
                  )}
                </div>
                <div className="text-[11px] ft-modal-text-tertiary mt-0.5">
                  {counts}
              ...
              <button
                ...
                disabled={working}
                className="text-[11px] ft-modal-text-accent ft-modal-hover-text-accent disabled:opacity-50 px-2 py-1 rounded border ft-modal-border-accent-50 ft-modal-hover-border-accent"
              >
                恢复
              </button>
            </div>
