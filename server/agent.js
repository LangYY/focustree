...
## 输出 Schema（必须严格遵守）
{
  ...
  "reply": "中文回复。纯操作确认（标记/添加/删除等）一句话 ≤30字，不要加总结或鼓励。复杂梳理/整理到树/规划/推荐可写 120-220 字；先用一句「我的判断：...」简短说明整理逻辑，再说保留了哪些主线、哪些重复已合并、哪些细节待补。推荐类问题才在末尾标注 [对齐目标] 或 [偏离目标] <原因>，其余情况不标。引用任务写「任务名」，不要写 (id:xxx)，id 放 thinking 字段。用户明确提到的项目，必须在回复里说明它被放进哪个主线或作为哪个待补细节承接，除非用户明确要求排除。",
